...
func (h *HTTPTransport) Provision(ctx caddy.Context) error {
	...
	if h.ClientCertificateMatcher.Name == "" {
		return fmt.Errorf("client_certificate_match must have either 'name'")
	}

	clientCert, err := h.ClientCertificateMatcher.getCertificate()
	if err != nil {
		return fmt.Errorf("no client certificate found in: %s with common name:", h.ClientCertificateMatcher.Location, h.ClientCertificateMatcher.Name)
	}
